...
unsigned char * loadBMPRaw(const char * imagepath, unsigned int& outWidth, unsigned int& outHeight, bool flipY){
    ...
    if (flipY){
        ...
        for (int i=0;i<outHeight/2;i++){
            // copy row i to tmp
            strcpy_s(tmpBuffer,size,(data+outWidth*3*i),size);
            // copy row h-i-1 to i
            memccpy(data+outWidth*3*i, size, data+outWidth*3*(outHeight-i-1), size);
            // copy tmp to row h-i-1
            memccpy(data+outWidth*3*(outHeight-i-1), size,tmpBuffer, size);
        }
        delete [] tmpBuffer;
    }
